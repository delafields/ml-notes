...
def load_amazon():
    filename = '..\data\Video_Games_5.json'
    train_summary, test_summary = [], []
    train_review_text, test_review_text = [], []
    ...
    with open(filename, 'r') as f:
        for (i, line) in enumerate(f):
            ...
            if data['overall'] == 3:
                ...
                raise Exception(f"Unexpected value {data['overall']}")
...
def load_amazon_smaller():
    ...
    return (train_summary[:size], train_review_text[:size], train_labels[:size]), (test_summary[:size], test_review_text[:size], test_labels[:size])